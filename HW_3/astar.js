    ...
    if (!closed.has(state_to_uniqueid(current.state))) {
      ...
      if (is_goal_state(current.state)) {
        ...
        while (successors.length != 0) {
          ...
          let real_path_cost = current.path_cost + fixed_step_cost;

          open.add({
            state: element.resultState,
            predecessor: predecessor_path,
            action: action_path,
            path_cost: real_path_cost,
            estimated_total_cost: real_path_cost + calculate_heuristic(element.resultState) 
          })
        }
